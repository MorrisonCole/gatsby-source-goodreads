...
exports.sourceNodes = ({ actions, createNodeId }, configOptions) => {
  ...
  const unpack = (obj) => {
    ...
    for (var key in obj) {
      ...
      if (
        typeof unpacked[key] === "object" &&
        unpacked[key].hasOwnProperty("_")
      ) {
        unpacked[key] = unpacked[key]["_"];
      }
      if (
        typeof unpacked[key] == "object" &&
        unpacked[key].hasOwnProperty("$")
      ) {
        unpacked[key] = unpacked[key]["$"];
      }
      if (
        typeof unpacked[key] === "object" &&
        !Array.isArray(unpacked[key]) &&
        !unpacked[key].hasOwnProperty("_")
      ) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    while (!sourceDone) {
      ...
      xml.parseString(raw, { trim: true }, (error, result) => {
        ...
        if (
          !(
            result.hasOwnProperty("GoodreadsResponse") &&
          ...
          return reject(
            "Invalid response from Goodreads: unexpected XML structure"
          );
        }
